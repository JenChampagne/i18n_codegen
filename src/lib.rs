...
// todo: Change this to have named properties.
/// Contains the Pascalized type name, and the original filename.
#[derive(Clone, Debug, Eq, PartialEq, Hash)]
struct LocaleName(String, String);

impl LocaleName {
    #[cfg(test)]
    fn new<T: Into<String>>(t: T) -> LocaleName {
        let s = t.into();
        LocaleName(s, s.clone())
    }
}
...
fn gen_locale_enum(locales: Vec<LocaleName>, out: &mut TokenStream) {
    ...
    // The following four sets of match arms are kept separate to check for
    // each variant style one at a time. This has two benefits.
    // 1. The original case and hyphen of the filename matches the fastest.
    // 2. We can avoid accidentally creating extra duplicate match arms.

    let original_arms = locales.iter().map(|key| {
        let ident = ident(key.0.as_str());
        let original = key.1.as_str();
    ...
    let lowercase_arms = locales.iter().map(|key| {
        let ident = ident(key.0.as_str());
        let lowercase = key.1.to_lowercase();

        // No point having an extra match arm if already lowercase.
        if lowercase.as_str() == key.1.as_str() {
            None
        } else {
            Some(quote! {
                #lowercase => Ok(#ident),
    ...
    // In same cases an input locale is hyphenated to specify region.
    // Some systems with hyphenate with a dash, others with an underscore.
    // Since a dash does not make a valid identifier in Rust, the locales
    // generated by this macro must be named with underscores, however,
    // we may find it useful to parse from strings wit hdashes.
    //
    // Some locales have multiple hyphens, but I did not encounter any
    // locales that mixed underscores and dashes together, and so this
    // feels like a good enough solution to avoid run-time string
    // allocation or pattern matching like regular expressions.
    let translated_original_arms = locales.iter().map(|key| {
        let ident = ident(key.0.as_str());
        let translated = key.1.replace("_", "-");

        // No point having an extra match arm if no hyphen translation needed.
        if translated.as_str() == key.1.as_str() {
            None
        } else {
            Some(quote! {
                #translated => Ok(#ident),
    ...
    let translated_lowercase_arms = locales.iter().map(|key| {
        let ident = ident(key.0.as_str());
        let lowercase = key.1.to_lowercase();
        let translated = lowercase.replace("_", "-");

        // No point if already lowercase or same as original.
        if lowercase.as_str() == translated.as_str() || lowercase.as_str() == key.1.as_str() {
            None
        } else {
            Some(quote! {
                #translated => Ok(#ident),
    ...
    out.extend(quote! {
        ...
        /// LocaleError enum generated by "i18n_codegen"
        #[derive(Copy, Clone, Debug)]
        pub enum LocaleError {
            UnknownLocale,
        }

        /// Locale parsing generated by "i18n_codegen"
        impl ::core::str::FromStr for Locale {
            ...
            fn from_str(s: &str) -> Result<Self, Self::Err> {
                ...
                match s {
                    #(#original_arms)*
                    #(#lowercase_arms)*
                    #(#translated_original_arms)*
                    #(#translated_lowercase_arms)*

                    _ => Err(Self::Err::UnknownLocale),
                }
            }
        }
    });
...
fn locale_name_from_translations_file_path(path: &PathBuf) -> Result<LocaleName> {
    ...
    Ok(LocaleName(name, file_stem.to_owned()))
}
...
mod test {
    ...
    fn test_building_translations() {
        ...
        assert_eq!(
            (translations[&Key("greeting".to_string())]
                [&LocaleName("En".to_string(), "en".to_string())]
                .0)
                .0,
            "Hello {name}",
        );
